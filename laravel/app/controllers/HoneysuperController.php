...
class HoneysuperController extends \BaseController {
	...
	public function index()
	{
		...
		$response 	= $client->get( [ 'url' => "http://api.mellifera.cu.cc/atomic/atomic/honeysupers" ] );
	...
	{
		$client 	= new HttpClient;
		$response 	= $client->get( [ 'url' => "http://api.mellifera.cu.cc/atomic/atomic/honeysupers/" . $id ] );
		$view 		= BeeTools::is_error( $response );
		if( $view ){
			return $view;
		}
