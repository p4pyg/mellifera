...
class HoneysuperController extends \BaseController
{
    ...
    public function index()
    {
        ...
        $response 	= $client->get( [ 'url' => Config::get( 'app.api' ) . "atomic/honeysupers", 'headers' 	=> ['Content-type: application/json','APIKEY:' . \Session::get( 'api_token' ) ] ] );
    ...
    {
        $client 	= new HttpClient;
        $response 	= $client->get( [ 'url' => Config::get( 'app.api' ) . "atomic/honeysupers/" . $id ], 'headers' 	=> ['Content-type: application/json','APIKEY:' . \Session::get( 'api_token' ) ] );
        $view 		= BeeTools::is_error( $response );
        if( $view ){
            return $view;
        }
