...
class QueenController extends \BaseController
{
    ...
    public function index()
    {
        ...
        if ($view) {
            return $view;                                                                                               // Retour de la vue d'erreur
        }
        $queens     = $response->json();
        $hives      = Hive::get();

        return View::make('queens.index', [ "queens" => $queens, "hives" => $hives ]);
    }
    ...
    public function store()
    {
        ...
        $response               = BeeTools::entityStore($queen, 'queens');                                                   // Enregistrement de la nouvelle reine
    ...
    {
        BeeTools::entityDelete($index, 'queens');
        return Redirect::to('queens');
    }

    /**
     * Déplacer ou affecter une reine à une ruche
     * @return  Redirect
     */
    public function transfert()
    {
        $inputs = Input::only('queen','hive');

        extract($inputs);

        $request = [
                    'url'       => Config::get('app.api') . 'atomic/association',
                    'params'    => json_encode(['queen' => $queen, 'beehive' => $hive]),
                    'headers'   => ['Content-type: application/json','APIKEY:' . \Session::get('api_token') ]
                ];
